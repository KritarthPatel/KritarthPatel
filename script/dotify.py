...
THEMES = {
    ...
    "dark": ("#ffffff", "#8b949e", None),
    "light": ("#1f2328", "#d0d7de", None),
}
...
def svg_header(w, h, rows, opts):
    ...
    return (
        ...
        f'aria-label="dot-matrix portrait">{style}{defs}'
        f'<g clip-path="url(#rounded-box)">'
        f'{bgrect}'
...
def main(argv=None):
    ...
    for theme in themes:
        ...
        # Note: Closing both <g> tags properly ensures valid SVG XML
        svg = svg_header(w, h, rows, args) + body + "</g></g></svg>"
